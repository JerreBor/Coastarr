<template>
  <div id="navbar">
    <nav class="flex items-center justify-between flex-wrap bg-transparent p-6">
      <div class="flex items-center fle-xshrink-0 text-white mr-6 ml-4">
        <RouterLink to="/">
          <Logo :color="'light'" :background="'trans'" :width="50" :height="50" />
        </RouterLink>
      </div>
      <div class="w-full block flex-grow lg:flex lg:items-center lg:w-auto mt-3 text-sm">
        <div class="lg:flex-grow">
          <RouterLink :class="currentRouteName.toLowerCase() == 'recommended' ? 'enabled' : ''" to="recommended" class="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-gray-600 mr-1 ml-6">
            RECOMMENDED
          </RouterLink>
          <RouterLink to="movies" :class="currentRouteName.toLowerCase() == 'movies' ? 'enabled' : ''" class="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-gray-600 mr-1 ml-6">
            MOVIES
          </RouterLink>
          <RouterLink to="series" :class="currentRouteName.toLowerCase() == 'series' ? 'enabled' : ''" class="block mt-4 lg:inline-block lg:mt-0 text-white hover:text-gray-600 mr-1 ml-6">
            SERIES
          </RouterLink>
        </div>
        <div>
          <RouterLink to="catalog" :class="currentRouteName.toLowerCase() == 'catalog' ? 'enabled' : ''" class="block lg:inline-block ml-6 mt-3 lg:ml-0 lg:mt-0 text-white hover:text-gray-600" style="font-size: 0.850rem;">
            CATALOG
          </RouterLink>
          <RouterLink to="#" class="block lg:inline-block ml-6 mt-2 lg:ml-6 lg:-mt-4">
            <svg
              id="Capa_1"
              version="1.1"
              ...
              y="0px"
              viewBox="0 0 480 480"
              style="enable-background:new 0 0 480 480; width:35; heigth:35; fill:white; transform: translate(0, 10px);"
              xml:space="preserve"
            >
              <g>
                <g>
